...
export function getChartOption(chart, data, isPdf) {
  ...
  const option = {
    ...
    grid: {
      // top: "30%",
      right: isChartTypeBarFull(chart) ? "5%" : isChartTypeBarTypical(chart)?"10%":"5%",
      containLabel: true,
    ...
    legend: {
      data: legendData,
      // top: "10%",
      orient: isChartTypeBarTypical(chart)?"vertical":"horizontal",
      x: isChartTypeBarTypical(chart)?"right":"center",
      y: isChartTypeBarTypical(chart)?"center":"bottom",
     },
    xAxis: {
      name: isChartTypeBarBasic(chart)? chart.scopes[0].text.split("").join("\n\n"):isChartTypeBarTypical(chart)?"所\n占\n人\n数\n百\n分\n比\n(\n%\n)":null,
      nameLocation: "middle",
      nameRotate: 0,
      nameGap: 50, 
      axisLine: {
      ...
      axisLabel: {
        fontSize: fontSize,
        formatter: isChartTypeBarBasic(chart) ? '{value}' :isChartTypeBarTypical(chart)? '{value}':'{value}%',
      },
      axisTick: {
        show: false,
      },
      ...
      min: isChartTypeBarBasic(chart) ? 0 : isChartTypeBarTypical(chart)? 0:0,
      max: isChartTypeBarBasic(chart) ? 5 : isChartTypeBarTypical(chart)? 100:100,
      interval: isChartTypeBarBasic(chart) ? 1 :isChartTypeBarTypical(chart)?10:10,
    },
    yAxis: [{
      name:isChartTypeBarTypical(chart) ?"分值":null,
      nameLocation:isChartTypeBarTypical(chart) ? "middle":null,
      nameRotate: 0,
      nameGap:isChartTypeBarTypical(chart)?40:null,
      data: yData,
